...
const AdminPanel = ({ navigation }) => {
    ...
    // FUNCIÓN: Convertir plan guardado al formato de visualización (MEJORADA - sincronizada con AdminCompanyDetailScreen)
    const convertStoredPlanToDisplayFormat = (storedPlan, companyData) => {
        console.log('🔄 [AdminPanel] Convirtiendo plan guardado al formato de visualización:', storedPlan);
        ...
        if (typeof storedPlan === 'string') {
            ...
            if (mappedPlan) {
                console.log('✅ [AdminPanel] Plan mapeado desde string:', storedPlan, '→', mappedPlan.name);
                return {
                    ...mappedPlan,
        ...
        if (typeof storedPlan === 'object' && storedPlan !== null) {
            console.log('✅ [AdminPanel] Plan es objeto, usando directamente:', storedPlan);
        ...
        if (companyData.monthlyAmount) {
            ...
            if (monthlyPrice === 499) {
                console.log('✅ [AdminPanel] Plan detectado por precio: Plan 3 Meses (499€)');
                return { ...planMappings['plan_3_months'], totalPrice: 499 * 3 };
            }
            if (monthlyPrice === 399) {
                console.log('✅ [AdminPanel] Plan detectado por precio: Plan 6 Meses (399€)');
                return { ...planMappings['plan_6_months'], totalPrice: 399 * 6 };
            }
            if (monthlyPrice === 299) {
                console.log('✅ [AdminPanel] Plan detectado por precio: Plan 12 Meses (299€)');
                return { ...planMappings['plan_12_months'], totalPrice: 299 * 12 };
            }
        }

        // Fallback al plan por defecto
        console.log('⚠️ [AdminPanel] Plan no reconocido, usando plan por defecto:', storedPlan);
        return {
    ...
    // FUNCIÓN: Detectar estado de pago inteligente (misma lógica que AdminCompanyDetailScreen)
    const getCompanyPaymentStatus = (companyData) => {
        if (!companyData) return 'pending';

        console.log('🔍 [AdminPanel] Detectando estado de pago para:', companyData.companyName);

        // CRITERIO 1: Estado explícito de pago completado
        if (companyData.status === 'payment_completed' || companyData.status === 'active') {
            console.log('✅ [AdminPanel] Estado explícito: pago completado');
            return 'completed';
        }
        ...
        if (companyData.firstPaymentCompletedDate || companyData.paymentCompletedDate) {
            console.log('✅ [AdminPanel] Fecha de pago completado encontrada');
            return 'completed';
        }
        ...
        if (companyData.selectedPlan && companyData.paymentMethodName && companyData.paymentMethodName !== 'No definido') {
            console.log('✅ [AdminPanel] Plan y método de pago definidos - asumiendo pago completado');
            return 'completed';
        }
        ...
        if (companyData.selectedPlan && companyData.monthlyAmount && companyData.monthlyAmount > 0) {
            console.log('✅ [AdminPanel] Plan y precio definidos - proceso Stripe completado');
            return 'completed';
        }
        ...
        if (companyData.selectedPlan && (companyData.selectedPlan.includes('plan_') || companyData.selectedPlan.includes('Plan'))) {
            console.log('✅ [AdminPanel] Plan válido detectado - entorno de prueba, asumiendo completado');
            return 'completed';
        }

        console.log('⚠️ [AdminPanel] No se detectó pago completado - estado pendiente');
        return 'pending';
    };

    // FUNCIÓN: Obtener estado de empresa para tarjetas
    ...
    // FUNCIÓN: Obtener texto del estado de empresa
    const getCompanyStatusText = (company) => {
        const status = getCompanyStatus(company);
        return status === 'active' ? 'Activa' : 'Pendiente';
    };

    // FUNCIÓN: Verificar si empresa está activa (para cálculos de dashboard)
    const isCompanyActive = (company) => {
        return getCompanyPaymentStatus(company) === 'completed';
    };

    // FUNCIÓN: Obtener información del plan para mostrar en tarjetas de empresa (SINCRONIZADA CON DETALLES)
    const getCompanyDisplayPlan = (company) => {
        if (!company) {
            console.log('⚠️ [AdminPanel-Cards] No hay datos de empresa, usando plan por defecto');
            return {
                name: 'Plan 6 Meses',
                price: 399,
                duration: 6,
        ...
        console.log('🔍 [AdminPanel-Cards] Obteniendo plan para tarjeta de empresa:', company.companyName);
        console.log('📋 [AdminPanel-Cards] Datos disponibles:', {
        ...
        // PRIORIDAD 1: Usar selectedPlan con mapeo completo (MISMA LÓGICA QUE DETALLES)
        if (company.selectedPlan) {
            console.log('✅ [AdminPanel-Cards] Usando selectedPlan:', company.selectedPlan);
            return convertStoredPlanToDisplayFormat(company.selectedPlan, company);
        }

        // PRIORIDAD 2: Detectar plan por precio mensual si está disponible (SINCRONIZADO)
        if (company.monthlyAmount) {
            const price = Number(company.monthlyAmount);
            const duration = company.planDuration || 6;

            console.log(`💰 [AdminPanel-Cards] Detectando por precio mensual: ${price}€, duración: ${duration} meses`);

            if (price === 499) {
                console.log('✅ [AdminPanel-Cards] Plan detectado: Plan 3 Meses (499€)');
                return { name: 'Plan 3 Meses', price: 499, duration: 3, totalPrice: 1497 };
            }
            if (price === 399) {
                console.log('✅ [AdminPanel-Cards] Plan detectado: Plan 6 Meses (399€)');
                return { name: 'Plan 6 Meses', price: 399, duration: 6, totalPrice: 2394 };
            }
            if (price === 299) {
                console.log('✅ [AdminPanel-Cards] Plan detectado: Plan 12 Meses (299€)');
                return { name: 'Plan 12 Meses', price: 299, duration: 12, totalPrice: 3588 };
            }

            // Si el precio no coincide con los estándar, usar el precio detectado
            console.log(`⚠️ [AdminPanel-Cards] Precio no estándar detectado: ${price}€`);
            return {
        ...
        // PRIORIDAD 3: Usar planId si está disponible (SINCRONIZADO)
        if (company.planId) {
            console.log('✅ [AdminPanel-Cards] Usando planId:', company.planId);
            return convertStoredPlanToDisplayFormat(company.planId, company);
        }

        // PRIORIDAD 4: Usar totalAmount si está disponible para calcular
        if (company.totalAmount) {
            const total = Number(company.totalAmount);
            const duration = company.planDuration || 6;
            const monthlyPrice = Math.round(total / duration);

            console.log(`💰 [AdminPanel-Cards] Calculando desde total: ${total}€ / ${duration} meses = ${monthlyPrice}€/mes`);

            return {
                name: `Plan ${duration} Meses`,
                price: monthlyPrice,
                duration: duration,
                totalPrice: total
            };
        ...
        // FALLBACK: Plan por defecto con datos disponibles (SINCRONIZADO)
        const fallbackPrice = 399;
        const fallbackDuration = 6;

        console.log('⚠️ [AdminPanel-Cards] Usando plan por defecto para empresa:', company.companyName);
    ...
    const loadAdminData = async () => {
        try {
            ...
            // 🛠️ SOLUCIÓN AUTOMÁTICA DE DUPLICADOS - EJECUTAR SIEMPRE
                        ...
                        try {
                            ...
                            if (subscriptionData) {
                                return {
                                    ...
                                    // Usar fecha de primer pago si está disponible, sino fecha de registro
    ...
    const handleApproveInfluencer = async (influencerId) => {
        try {
            ...
            if (result) {
                ...
                Alert.alert('Éxito', 'Influencer aprobado correctamente');
    ...
    const handleDeleteInfluencerAccount = async (influencerId, influencerName) => {
        try {
            ...
            Alert.alert(
                '🗑️ Eliminar Cuenta (GDPR)',
                `¿Estás seguro de que quieres eliminar PERMANENTEMENTE la cuenta de ${influencerName}?\n\n⚠️ ADVERTENCIA GDPR:\n• Todos los datos personales serán borrados\n• Las credenciales de acceso serán eliminadas\n• Esta acción NO se puede deshacer\n• Cumple con el derecho al olvido (GDPR)\n\nEsta es una eliminación completa y definitiva.`,
                [
                    {
                        text: 'Cancelar',
                        style: 'cancel'
                        ...
                        onPress: async () => {
                            try {
                                ...
                                if (result.success) {
                                    ...
                                    Alert.alert('❌ Error en Eliminación GDPR', result.message);
                                }
                            } catch (error) {
            ...
            Alert.alert('❌ Error', 'No se pudo iniciar la eliminación de la cuenta');
        }
    };
    ...
    const handleDeleteCompanyAccount = async (companyId, companyName) => {
        try {
            // Show GDPR confirmation dialog
            Alert.alert(
                '🗑️ Eliminar Empresa (GDPR)',
                `¿Estás seguro de que quieres eliminar PERMANENTEMENTE la empresa "${companyName}"?\n\n⚠️ ADVERTENCIA GDPR:\n• Todos los datos empresariales serán borrados\n• Las credenciales de acceso serán eliminadas\n• Los pagos futuros serán cancelados\n• Esta acción NO se puede deshacer\n• Cumple con el derecho al olvido (GDPR)\n\n💳 IMPORTANTE: Se cancelarán automáticamente todos los pagos recurrentes y suscripciones activas.\n\nEsta es una eliminación completa y definitiva.`,
                [
                    {
                        text: 'Cancelar',
                        style: 'cancel'
                    },
                    {
                        ...
                        onPress: async () => {
                            try {
                                console.log(`🗑️ Iniciando eliminación GDPR para empresa: ${companyName}`);
                                ...
                                if (result.success) {
                                    ...
                                    Alert.alert(
                                        '✅ Eliminación GDPR Completada',
                                        `La empresa "${companyName}" ha sido eliminada permanentemente.\n\n🔒 GDPR Compliance:\n• ✅ Datos empresariales borrados\n• ✅ Credenciales eliminadas\n• ✅ Suscripciones canceladas\n• ✅ Pagos futuros cancelados\n• ✅ Acceso revocado\n\nLa empresa ya no puede acceder a la aplicación y no se realizarán más cobros.`,
                                        [{ text: 'Entendido', style: 'default' }]
                                    );
                                ...
                                Alert.alert('❌ Error', 'No se pudo completar la eliminación GDPR de la empresa');
                            }
                        }
                    }
                ]
            ...
            Alert.alert('❌ Error', 'No se pudo iniciar la eliminación de la empresa');
        }
    };
    ...
    // 🧹 FUNCIÓN TEMPORAL DE DEBUG - REMOVER EN PRODUCCIÓN
    const handleClearPasswordCache = async () => {
        try {
            Alert.alert(
                '🧹 Limpiar Cache',
                '¿Estás seguro de que quieres limpiar el cache de contraseñas? Esto reseteará el sistema.',
                [
                    { text: 'Cancelar', style: 'cancel' },
                    {
                        ...
                        onPress: async () => {
                            ...
                            if (result) {
                                Alert.alert('✅ Cache Limpiado', 'Reinicia la app para aplicar cambios.');
                            } else {
    ...
    const handleAdminChangePassword = async () => {
        try {
            ...
            // Verificar contraseña actual usando el sistema mejorado
            console.log('🔐 [ADMIN] Verificando contraseña actual...');
            const currentStoredPassword = await StorageService.getAdminPassword();
            console.log('🔐 [ADMIN] Contraseña almacenada:', currentStoredPassword);
            console.log('🔐 [ADMIN] Contraseña introducida:', changePasswordForm.currentPassword);

            if (currentStoredPassword !== changePasswordForm.currentPassword) {
                console.log('❌ [ADMIN] Contraseña actual incorrecta');
                Alert.alert('Error', 'La contraseña actual es incorrecta');
                setIsChangingPassword(false);
                return;
            }

            console.log('✅ [ADMIN] Contraseña actual verificada');

            // Guardar nueva contraseña usando el sistema mejorado
            console.log('🔐 [ADMIN] Guardando nueva contraseña...');
            const saveResult = await StorageService.saveAdminPassword(changePasswordForm.newPassword);

            if (!saveResult) {
                console.error('❌ [ADMIN] Error al guardar la nueva contraseña');
                Alert.alert('Error', 'No se pudo guardar la nueva contraseña. Inténtalo de nuevo.');
                setIsChangingPassword(false);
                return;
            }

            console.log('✅ [ADMIN] Nueva contraseña guardada exitosamente');

            // Verificar que la nueva contraseña se guardó correctamente
            console.log('🔐 [ADMIN] Verificando nueva contraseña...');
            const verificationPassword = await StorageService.getAdminPassword();

            if (verificationPassword !== changePasswordForm.newPassword) {
                console.error('❌ [ADMIN] Error en verificación de nueva contraseña');
                console.error('❌ [ADMIN] Esperada:', changePasswordForm.newPassword);
                console.error('❌ [ADMIN] Obtenida:', verificationPassword);
                Alert.alert('Error', 'Error en la verificación de la nueva contraseña. Inténtalo de nuevo.');
                setIsChangingPassword(false);
                return;
            }

            console.log('✅ [ADMIN] Nueva contraseña verificada correctamente');
            ...
            if (currentUser) {
                ...
                console.log('✅ [ADMIN] Usuario actualizado');
            }
            ...
            console.log('✅ [ADMIN] Cambio de contraseña completado exitosamente');

            Alert.alert(
                '✅ Contraseña Actualizada',
                `Tu contraseña de administrador ha sido cambiada exitosamente.\n\n🔐 Nueva contraseña activa: ${changePasswordForm.newPassword}\n\n⚠️ IMPORTANTE: Esta es ahora la ÚNICA contraseña válida para iniciar sesión como administrador. Guárdala en un lugar seguro.`,
                [{
                    text: 'Entendido',
                    onPress: () => {
                        console.log('🔐 [ADMIN] Usuario confirmó el cambio de contraseña');
                    }
                }]
            );

        } catch (error) {
            console.error('❌ [ADMIN] Error changing admin password:', error);
    ...
    const handleViewCompanyLocations = (company) => {
        try {
            console.log('📍 Navegando a locales de empresa:', company.companyName);
    ...
    const loadCitiesData = async () => {
        try {
            ...
            console.log('🏙️ [AdminPanel] Cargando datos de ciudades...');

            const citiesData = await CitiesService.getAllCities();
            setCities(citiesData);

            console.log('✅ [AdminPanel] Ciudades cargadas:', citiesData.length);
        } catch (error) {
            console.error('❌ [AdminPanel] Error cargando ciudades:', error);
    ...
    const handleSaveCity = async () => {
        try {
            ...
            if (result.success) {
                console.log('✅ [AdminPanel] Ciudad guardada exitosamente, eventos emitidos automáticamente');
            ...
            console.error('❌ [AdminPanel] Error guardando ciudad:', error);
    ...
    const handleDeleteCity = async (city) => {
        try {
            Alert.alert(
                'Eliminar Ciudad',
                `¿Estás seguro de que quieres eliminar "${city.name}"?\n\nEsta acción no se puede deshacer y la ciudad desaparecerá del selector de influencers.`,
                [
                    { text: 'Cancelar', style: 'cancel' },
                    {
                        text: 'Eliminar',
                        ...
                        onPress: async () => {
                            ...
                            if (result.success) {
                                console.log('✅ [AdminPanel] Ciudad eliminada exitosamente, eventos emitidos automáticamente');
                                Alert.alert('Éxito', result.message);
                                await loadCitiesData(); // Recargar datos
                            } else {
                                Alert.alert('Error', result.message);
            ...
            console.error('❌ [AdminPanel] Error eliminando ciudad:', error);
    ...
    const handleToggleCityStatus = async (city) => {
        try {
            ...
            if (result.success) {
                console.log('✅ [AdminPanel] Estado de ciudad cambiado exitosamente, eventos emitidos automáticamente');
                await loadCitiesData(); // Recargar datos
            } else {
                Alert.alert('Error', result.message);
            }
        } catch (error) {
            console.error('❌ [AdminPanel] Error cambiando estado de ciudad:', error);
        ...
        <View>
            ...
            <View style={styles.statsGrid}>
                <View style={styles.statCard}>
                    <Text style={styles.statNumber}>€{dashboard.totalRevenue.toLocaleString()}</Text>
                    <Text style={styles.statLabel}>Ingresos Totales Previstos</Text>
                </View>
                <View style={styles.statCard}>
                    <Text style={styles.statNumber}>€{dashboard.monthlyRevenue.toLocaleString()}</Text>
        ...
        <View>
            ...
            <View style={styles.financialSummary}>
                <View style={styles.revenueCard}>
                    <Text style={styles.revenueAmount}>€{dashboard.totalRevenue.toLocaleString()}</Text>
                    <Text style={styles.revenueLabel}>Ingresos Totales Previstos</Text>
                </View>
                <View style={styles.revenueCard}>
                    <Text style={styles.revenueAmount}>€{dashboard.monthlyRevenue.toLocaleString()}</Text>
    ...
    const renderCompanies = () => (
        <ScrollView style={styles.sectionContainer}>
            <Text style={styles.sectionTitle}>Gestión de Empresas</Text>
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View style={styles.companyCard}>
                        ...
                        <Text style={styles.companyInfo}>Teléfono: {item.companyPhone}</Text>
                        ...
                        {item.nextBillingDate && (
                            <Text style={styles.companyInfo}>
                                Próximo pago: {new Date(item.nextBillingDate).toLocaleDateString()}
                            </Text>
                        )}
                ...
                <FlatList
                    ...
                    renderItem={({ item }) => (
                        <View style={styles.influencerCard}>
                            ...
                            <Text style={styles.influencerInfo}>Teléfono: {item.phone}</Text>

                            {/* Botón para ver capturas de Instagram */}
                            <TouchableOpacity
                        ...
                        <View style={styles.influencerCard}>
                            ...
                            <View style={styles.accessStatus}>
                                <Text style={[styles.accessStatusText, item.hasAccess ? styles.accessActive : styles.accessInactive]}>
                                    {item.hasAccess ? '✅ Tiene acceso a la app' : '❌ Sin acceso a la app'}
                                </Text>
                            </View>

                            {/* Botón para eliminar cuenta (GDPR) */}
                            <TouchableOpacity
    ...
    const renderCampaigns = () => (
        <ScrollView style={styles.sectionContainer}>
            <Text style={styles.sectionTitle}>Gestión de Campañas</Text>

            {/* Create Campaign Form */}
            <View style={styles.createCampaignSection}>
                <Text style={styles.subsectionTitle}>Crear Nueva Campaña</Text>
            ...
            {/* Existing Campaigns */}
            <Text style={styles.subsectionTitle}>Campañas Existentes</Text>
            <FlatList
                data={campaigns.list}
                keyExtractor={(item) => item.id.toString()}
                renderItem={({ item }) => (
                    <View style={styles.campaignCard}>
                        ...
                        <Text style={styles.campaignInfo}>Categoría: {item.category}</Text>
                        <Text style={styles.campaignInfo}>Ciudad: {item.city}</Text>

                        <View style={styles.campaignActions}>
                            <TouchableOpacity
                                style={[styles.actionButton, styles.editButton]}
                                onPress={() => Alert.alert('Editar', 'Función de edición en desarrollo')}
                            >
    ...
    const renderSecurity = () => (
        <ScrollView style={styles.sectionContainer}>
            ...
            <View style={styles.securitySection}>
                <Text style={styles.securitySectionTitle}>Gestión de Contraseña</Text>
                <Text style={styles.securitySectionDescription}>
                    Mantén tu cuenta de administrador segura actualizando tu contraseña regularmente.
                </Text>
                ...
                >
                    <View style={styles.securityButtonContent}>
                        ...
                        <View style={styles.securityButtonTextContainer}>
                            <Text style={styles.securityButtonTitle}>Contraseña y Seguridad</Text>
                            <Text style={styles.securityButtonSubtitle}>Cambiar contraseña de administrador</Text>
                        </View>
                ...
                {currentUser?.lastPasswordChange && (
                    <Text style={styles.lastPasswordChange}>
                        Última actualización: {new Date(currentUser.lastPasswordChange).toLocaleDateString()}
                    </Text>
                )}
            </View>
            ...
            <View style={styles.securitySection}>
                ...
                <View style={styles.securityTip}>
                    ...
                    <Text style={styles.securityTipText}>Usa una contraseña fuerte con al menos 8 caracteres</Text>
                </View>
                <View style={styles.securityTip}>
                    <View style={styles.securityTipIcon}>
                        <MinimalistIcons name="settings" size={20} color="#C9A961" />
                    </View>
                    <Text style={styles.securityTipText}>Cambia tu contraseña cada 3-6 meses</Text>
                </View>
                <View style={styles.securityTip}>
                    <View style={styles.securityTipIcon}>
    ...
    const renderCities = () => (
        <ScrollView style={styles.sectionContainer}>
            <Text style={styles.sectionTitle}>Gestión de Ciudades</Text>
            ...
            <View style={styles.citiesSection}>
                ...
                <TouchableOpacity style={styles.addButton} onPress={handleAddCity}>
                    <Text style={styles.addButtonText}>+ Añadir Nueva Ciudad</Text>
                </TouchableOpacity>
    ...
    return (
        <SafeAreaView style={styles.container}>
            <LinearGradient colors={['#000000', '#1a1a1a']} style={styles.gradient}>
                    ...
                    <View style={styles.modalOverlay}>
                        <View style={styles.passwordModalContent}>
                            <View style={styles.passwordModalHeader}>
                                <Text style={styles.passwordModalTitle}>Contraseña y Seguridad</Text>
                                <TouchableOpacity
                            ...
                            <View style={styles.passwordModalIconContainer}>
                                ...
                                <Text style={styles.passwordModalSubtitle}>
                                    Cambia tu contraseña de administrador para mantener la seguridad
                                </Text>
                            </View>

                            <ScrollView style={styles.passwordModalForm}>
                                <View style={styles.passwordInputGroup}>
                                    <Text style={styles.passwordInputLabel}>Contraseña Actual</Text>
                                    <TextInput
                                ...
                                <View style={styles.passwordInputGroup}>
                                    <Text style={styles.passwordInputLabel}>Nueva Contraseña</Text>
                                    <TextInput
                                        style={styles.passwordTextInput}
                                ...
                                <View style={styles.passwordInputGroup}>
                                    <Text style={styles.passwordInputLabel}>Confirmar Nueva Contraseña</Text>
                                    <TextInput
                                        style={styles.passwordTextInput}
                    ...
                    <View style={styles.modalOverlay}>
                        <View style={styles.modalContent}>
                            ...
                            <Text style={styles.modalSubtitle}>
                                ¿Por qué rechazas a {selectedInfluencer?.fullName}?
                            </Text>
                ...
                {/* Modal de confirmación GDPR para eliminar cuenta de influencer */}
                <Modal
                    ...
                    <View style={styles.modalOverlay}>
                        <View style={styles.modalContent}>
                            ...
                            <View style={styles.modalButtons}>
                                ...
                                >
                                    <Text style={styles.modalButtonText}>🗑️ Eliminar Permanentemente</Text>
                                </TouchableOpacity>
                            </View>
                        </View>
                    </View>
                        ...
                        <View style={styles.cityModalContent}>
                            <View style={styles.cityModalHeader}>
                                <Text style={styles.cityModalTitle}>
                                    {cityModalMode === 'add' ? 'Añadir Nueva Ciudad' : 'Editar Ciudad'}
                                </Text>
                                <TouchableOpacity
                            ...
                            <View style={styles.cityModalBody}>
                                <Text style={styles.cityModalDescription}>
                                    {cityModalMode === 'add'
                                        ? 'La nueva ciudad aparecerá inmediatamente en el selector de influencers.'
                                        : 'Los cambios se aplicarán inmediatamente en el selector de influencers.'
                                    }
                                </Text>
                                ...
                                <View style={styles.cityModalActions}>
                                    ...
                                    >
                                        <Text style={styles.citySaveButtonText}>
                                            {cityModalMode === 'add' ? 'Añadir Ciudad' : 'Guardar Cambios'}
                                        </Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
